...
-- Variable global para trackear el último sender de correo
Accountant_LastMailSender = nil;
Accountant_SelectedMailIndex = nil;

-- Asegurar que las cadenas de localización críticas existen ANTES de usar funciones
if not ACCLOC_TITLE        then ACCLOC_TITLE        = "Accountant Classic" end
if not ACCLOC_NEWPROFILE   then ACCLOC_NEWPROFILE   = "Nuevo perfil de Accountant creado para" end
if not ACCLOC_LOADPROFILE  then ACCLOC_LOADPROFILE  = "Perfil de Accountant cargado para" end
if not ACCLOC_LOADED       then ACCLOC_LOADED       = "Cargado" end
if not ACCLOC_SOURCE       then ACCLOC_SOURCE       = "Origen" end
if not ACCLOC_IN           then ACCLOC_IN           = "Ingresos" end
if not ACCLOC_OUT          then ACCLOC_OUT          = "Gastos" end
if not ACCLOC_TOT_IN       then ACCLOC_TOT_IN       = "Ingresos totales" end
if not ACCLOC_TOT_OUT      then ACCLOC_TOT_OUT      = "Gastos totales" end
if not ACCLOC_NET          then ACCLOC_NET          = "Beneficio / Pérdida neta" end
if not ACCLOC_NETLOSS      then ACCLOC_NETLOSS      = "Pérdida neta" end
if not ACCLOC_NETPROF      then ACCLOC_NETPROF      = "Beneficio neto" end
if not ACCLOC_SUM          then ACCLOC_SUM          = "Suma total" end
if not ACCLOC_CHAR         then ACCLOC_CHAR         = "Personaje" end
if not ACCLOC_MONEY        then ACCLOC_MONEY        = "Dinero" end
if not ACCLOC_UPDATED      then ACCLOC_UPDATED      = "Actualizado" end
if not ACCLOC_RESET        then ACCLOC_RESET        = "Restablecer" end
if not ACCLOC_OPTBUT       then ACCLOC_OPTBUT       = "Opciones" end
if not ACCLOC_EXIT         then ACCLOC_EXIT         = "Salir" end
if not ACCLOC_SESS         then ACCLOC_SESS         = "Sesión" end
if not ACCLOC_DAY          then ACCLOC_DAY          = "Día" end
if not ACCLOC_WEEK         then ACCLOC_WEEK         = "Semana" end
if not ACCLOC_TOTAL        then ACCLOC_TOTAL        = "Total" end
if not ACCLOC_CHARS        then ACCLOC_CHARS        = "Cuenta" end
if not ACCLOC_WEEKSTART    then ACCLOC_WEEKSTART    = "Inicio de la semana" end
...
if not ACCLOC_MERCH  then ACCLOC_MERCH  = "Vendedores" end
if not ACCLOC_QUEST  then ACCLOC_QUEST  = "Recompensas de misiones" end
if not ACCLOC_TRADE  then ACCLOC_TRADE  = "Ventana de comercio" end
if not ACCLOC_MAIL   then ACCLOC_MAIL   = "Correo" end
if not ACCLOC_AUC    then ACCLOC_AUC    = "Casa de subastas" end
if not ACCLOC_TRAIN  then ACCLOC_TRAIN  = "Coste de entrenamiento" end
if not ACCLOC_TAXI   then ACCLOC_TAXI   = "Tarifas de vuelo" end
if not ACCLOC_REPAIR then ACCLOC_REPAIR = "Coste de reparación" end
if not ACCLOC_OTHER  then ACCLOC_OTHER  = "Desconocido" end
...
function Accountant_OnEvent(event)
	...
	elseif event == "MAIL_SHOW" then
		...
		Accountant_LastMailSender = nil;
		-- Capturar el remitente del primer correo con dinero
		local numItems = GetInboxNumItems();
		for i = 1, numItems do
			local packageInfo, stationeryIcon, sender, subject, money = GetInboxHeaderInfo(i);
			if sender and money and money > 0 then
				local senderLower = string.lower(sender);
				if string.find(senderLower, "casa de subastas") or 
				   string.find(senderLower, "auction house") or
				   string.find(senderLower, "subasta") then
					Accountant_LastMailSender = sender;
					if Accountant_Verbose then 
						ACC_Print("Correo de subasta detectado: " .. sender); 
					end
					break;
				end
			end
	...
	elseif event == "MAIL_INBOX_UPDATE" then
		-- Simplificar: solo actualizar si no tenemos sender ya
		if not Accountant_LastMailSender and InboxFrame and InboxFrame:IsVisible() then
			local numItems = GetInboxNumItems();
			for i = 1, numItems do
				local packageInfo, stationeryIcon, sender, subject, money = GetInboxHeaderInfo(i);
				if sender and money and money > 0 then
					...
					if string.find(senderLower, "casa de subastas") or 
					   ...
					   string.find(senderLower, "subasta") then
						Accountant_LastMailSender = sender;
						if Accountant_Verbose then 
							ACC_Print("Actualizado correo de subasta: " .. sender); 
						end
						break;
					end
				end
...
function Accountant_OnShow()
	...
	-- Forzar textos localizados en cabeceras y botones visibles
	AccountantFrameTitleText:SetText(ACCLOC_TITLE)
	AccountantFrameSource:SetText((Accountant_CurrentTab == 5) and ACCLOC_CHAR or ACCLOC_SOURCE)
	AccountantFrameIn:SetText((Accountant_CurrentTab == 5) and ACCLOC_MONEY or ACCLOC_IN)
	AccountantFrameOut:SetText((Accountant_CurrentTab == 5) and ACCLOC_UPDATED or ACCLOC_OUT)
	AccountantFrameResetButton:SetText(ACCLOC_RESET)
	AccountantFrameExitButton:SetText(ACCLOC_EXIT)
	AccountantFrameOptionsButton:SetText(ACCLOC_OPTBUT)
	-- Tabs (por si XML retiene el texto inicial)
	AccountantFrameTab1:SetText(ACCLOC_SESS)
	PanelTemplates_TabResize(AccountantFrameTab1, 10)
	AccountantFrameTab2:SetText(ACCLOC_DAY)
	PanelTemplates_TabResize(AccountantFrameTab2, 10)
...
function Accountant_UpdateLog()
	...
	-- Detectar si es dinero de casa de subastas por correo
	if mode == "MAIL" and diff > 0 and Accountant_LastMailSender then
		local sender = string.lower(Accountant_LastMailSender);
		if string.find(sender, "casa de subastas") or 
		   string.find(sender, "auction house") or
		   string.find(sender, "subasta") or
		   string.find(sender, "auctionhouse") then
			mode = "AH";
			if Accountant_Verbose then 
				ACC_Print("Reclasificado como ganancia de subasta: " .. Accountant_NiceCash(diff) .. " de " .. Accountant_LastMailSender); 
			end
		end
	end
